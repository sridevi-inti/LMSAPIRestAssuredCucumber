...
@When("User sends the post request using \"([^\"]*)\" as$")
public void user_sends_the_post_request_using_saveprogram_as(String url) {
	...
	saveprogram = response.getBody().jsonPath().getString("programName");
	System.out.println("Response status code: " + response.statusCode());
}

@Then("Status code should come as {int} ok")
public void status_code_should_come_ok(Integer int1) {
	int statusCode=((ResponseOptions<Response>) response).getStatusCode();
    Assert.assertEquals(201, statusCode);
}

@Then("User performs Delete operation to clear the porgram for the url \"([^\"]*)\"$")
public void user_performs_Delete_operation_to_clear_the_porgram_for_the_url(String url) {
	Response response = request.delete(url + "/" + saveprogram);
	...
	if(response.statusCode() == 201) {
  	  System.out.println("Save Program " + saveprogram + " has been deleted successfully.");
}
}
}
